...
MobileCatalogueDto mobileCatalogueDtoFromJson(String str) =>
    MobileCatalogueDto.fromJson(json.decode(str), null);
...
class MobileCatalogueDto {
  ...
  MobileCatalogueDto.fromJson(dynamic json, String? country) {
    // appLog("the description");
    // appLog(json['statusDescription']);
    _statusDescription = json['statusDescription'];
    if (json['catalogue'] != null) {
      _catalogue = [];
      json['catalogue'].forEach((v) {
        _catalogue?.add(Catalogue.fromJson(v, country));
      });
    }
...
Catalogue catalogueFromJson(String str) =>
    Catalogue.fromJson(json.decode(str), null);
...
class Catalogue {
  ...
  Catalogue.fromJson(dynamic json, String? country) {
    ...
    if (json['items'] != null) {
      ...
      json['items'].forEach((v) {
        _items?.add(Items.fromJson(v, country));
      });
    }
    _key = json['key'];
  }
...
Items itemsFromJson(String str) => Items.fromJson(json.decode(str), null);
...
class Items {
  ...
  Items.fromJson(dynamic json, String? country) {
    // appLog("we're good");
    // appLog(json['unitPrice'].runtimeType);
    // final countryManager = CountryManager();
    // // countryManager.loadCountryFromPreferences();
    // final String country = countryManager.country;
    appLog("the search is here");
    appLog("the search");
    appLog(country);
    // appLog(json['prices']);
    ...
    if (json['prices'] != null && json['prices'] is List) {
      ...
      if (country == 'ETB') {
        ...
        appLog("// For Ethiopia, find price with isMainCurrency = true)");
        appLog(_currency);
      } else if (country == "AE") {
        var mainPrice = prices.firstWhere(
        ...
        _unitPrice = foreignPrice['price'].toString();
        _currency = foreignPrice['currencyCode'];
        appLog("// For other countries, find price with isMainCurrency)");
        appLog(_unitPrice);
      }
    } else {
      // Fallback to old price format
      _unitPrice = json['unitPrice'].toString();
      _currency = json['currency'] ?? "";
    }

    // _unitPrice = json['unitPrice'].toString();
    // _currency = json['currency'];
    _actualPrice = _unitPrice;
    _prodId = json['ProductId'].toString();
    _offerPrice = num.tryParse(_unitPrice ?? '0');
    // appLog("we're good2");
  }
